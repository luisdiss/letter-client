...
class Contacts:
    def __init__(self, state):
        self.state = state
        self.contacts_filename: str = "contacts.txt"
        self.name_to_contact: dict[str, Contact] = self._load_contacts()

    def _load_contacts(self) -> dict[str, Contact]:
        try:
            with open(self.contacts_filename, 'r') as file:
                return json.load(file, object_hook=Contact.deserialise)
        except FileNotFoundError:
            with open(self.contacts_filename, "w") as file:
                file.write("{}")
        except PermissionError: 
            print(f"permission error: could not load contacts at location: {self.contacts_filename}")
            sys.exit()
    ...
    def _save_contacts(self) -> bool:
        try:
            ...
            print(f"file not found error: could not save contacts at location: {self.contacts_filename}")
        except PermissionError:
            print(f"permission error: could not save contacts to location: {self.contacts_filename}")
        return False
    ...
    def add_contact(self, username: str, name: str) -> None:
        ...
        resp = rq.head(self.state.settings["server_url"] + f"/users/{username}", headers=headers)
